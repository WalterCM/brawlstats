...
export const api = {
  ...
  // Fetch club stats
  async fetchClubStats(clubId, sortBy = 'win_rate', filters = {}) {
    let url = `${API_BASE_URL}/clubs/${clubId}/stats/?sort_by=${sortBy}`;
    if (filters.mode && filters.mode !== 'All') url += `&mode=${encodeURIComponent(filters.mode)}`;
